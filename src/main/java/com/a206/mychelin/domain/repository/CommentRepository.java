...
public interface CommentRepository extends JpaRepository<Comment, String> {
    ...
    @Query(value = "SELECT c.comment_id, u.nickname, c.message, c.create_date FROM post p, comment c, user u WHERE p.id = c.post_id and p.id = :post_id and u.id = c.writer_id order by c.create_date desc limit 2", nativeQuery = true)
    List<Object[]> findCommentsLimit2(@Param("post_id") int postId);
